...
def equals():
    global equation_text
    try:
        # Replace 'π' with its value to 10 decimal places before evaluating
        equation_to_eval = equation_text.replace('π', str(format(math.pi, '.10f')))
        equation_to_eval = equation_to_eval.replace('√(', 'math.sqrt(')
        if equation_to_eval.count('(') == equation_to_eval.count(')'):
            total = str(eval(equation_to_eval))
...
bracketOpen = Button(frame, text='(', height=4, width=9, font=35, command=lambda: button_press('('))
bracketOpen.grid(row=5, column=0)

pi_button = Button(frame, text='π', height=4, width=9, font=35, command=pi_press)
pi_button.grid(row=5, column=1)

clear = Button(window, text='clear', height=4, width=12, font=35, command=clear)
clear.pack()

bracketClose = Button(frame, text=')', height=4, width=9, font=35, command=lambda: button_press(')'))
bracketClose.grid(row=5, column=3)

sqrt_button = Button(frame, text='√', height=4, width=9, font=35, command=sqrt_press)
